              ...
              {NAV_LINKS.map((link) => (
                <a
                  ...
                  onClick={(e) => {
                    ...
                    } else if (link.name === 'Results') {
                      e.preventDefault();
                      window.open(import.meta.env.BASE_URL + 'results.html', '_blank');
                    } else {
        ...
        {isOpen && !isDesktop && (
          <motion.div initial={{ opacity: 0, clipPath: 'circle(0% at 90% 10%)' }} animate={{ opacity: 1, clipPath: 'circle(150% at 90% 10%)' }} exit={{ opacity: 0, clipPath: 'circle(0% at 90% 10%)' }} transition={{ duration: 0.6, ease: [0.19, 1, 0.22, 1] }} className="fixed inset-0 bg-black/95 z-50 flex flex-col items-center justify-center space-y-12 overflow-hidden">
            ...
            {NAV_LINKS.map((link, idx) => (
              <motion.a key={link.name} href={link.href} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * idx }} className={`text-4xl text-zinc-300 hover:text-stranger-red font-stranger uppercase tracking-[0.15em] relative group text-center`} onClick={(e) => {
                if (link.name === 'Problem Statements') {
                  e.preventDefault();
                  window.open(import.meta.env.BASE_URL + 'problem-statements.html', '_blank');
                } else if (link.name === 'Bounties') {
                ...
                <span className="relative z-10">{link.name}</span>
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 bg-stranger-red scale-0 group-hover:scale-100 transition-transform rounded-full blur-[2px]" />
              </motion.a>
            ))}
          </motion.div>
        )}
      ...
      {!isDesktop && (
        ...
        <div className="relative space-y-6">
          ...
          {[
            ...
            { title: "Last date to register", date: "17 April", desc: "The gate closes soon. This is your final chance to establish contact. After this date, the portal seals. No more teams can cross between dimensions." },
            ...
            { title: "Results Out", date: "28 April", desc: "The truth emerges from static. Champions will be revealed. Those who conquered the Upside Down and returned with the most powerful artifacts will be crowned. The end... or a new beginning?" }
          ].map((event, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: idx * 0.1 }} className="relative">
              <div className="bg-black/70 border-2 border-orange-600/60 backdrop-blur-md p-4 sm:p-6 rounded-xl">
                <h3 className="text-base sm:text-lg font-bold text-white mb-2 font-serif uppercase">{event.title}</h3>
                <p className="text-orange-500 font-black text-lg sm:text-xl mb-3">{event.date}</p>
                <p className="text-zinc-300 text-xs sm:text-sm font-serif leading-relaxed">{event.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-6 mb-12">
          ...
          <motion.div initial={{ opacity: 0, y: -30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="flex justify-center">
            <div className="bg-black/70 border-2 border-orange-600/60 backdrop-blur-md p-8 rounded-xl text-center w-full shadow-[0_0_30px_rgba(234,88,12,0.2)] hover:shadow-[0_0_40px_rgba(234,88,12,0.4)] transition-all duration-500">
              ...
              <p className="text-orange-500 font-black text-2xl mb-4">17 April</p>
        ...
        <div className="grid grid-cols-2 gap-6 max-w-2xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className="flex justify-center">
            <div className="bg-black/70 border-2 border-orange-600/60 backdrop-blur-md p-8 rounded-xl text-center w-full shadow-[0_0_30px_rgba(234,88,12,0.2)] hover:shadow-[0_0_40px_rgba(234,88,12,0.4)] transition-all duration-500">
              ...
              <p className="text-orange-500 font-black text-2xl mb-4">28 April</p>
